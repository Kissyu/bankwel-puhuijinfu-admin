...
<div class="container-fluid" style="height: 650px; overflow-y: auto;">
    ...
    <!-- 添加信息数据区 start -->
    <div class="row-fluid">
        <form id="mainForm" class="form-horizontal alert alert-info fade in span12" action="/phjfht/api/v1/bankinfo/modifyLoanInfo" method="POST">
            <!-- 提示信息标签 -->
            ...
            <fieldset>
                <div class="row">
                    ...
                    <webTag:BankTypeSelectTag id="bt_code" name="bt_code" displayLable="银行类型" value='${data.bt_code}' isRequired="true"/>
                    <webTag:DataDictionarySelectTag sys_id="Phjf" id="loan_plate" name="loan_plate" displayLable="所属版块" parentCode="loan_plate" value='${data.loan_plate}' isRequired="true" />
                </div>
                <div class="row">
                    <webTag:DataDictionarySelectTag sys_id="Phjf" id="loan_use_type" name="loan_use_type" displayLable="用途" parentCode="loan_useType" value='${data.loan_use_type}' isRequired="true" />
                    <webTag:Text id="name" name="name" displayLable="产品名称" value='${data.name}' isRequired="true" maxlength="100"/>
                    <webTag:Text id="tags" name="tags" displayLable="产品关键字" value='${data.tags}' isRequired="true" maxlength="255" />
                </div>
                <div class="row">
                    <webTag:ImgUploadTag id="logo" name="logo" displayLable="宣传图片" value="${data.logo}"/>
                    <webTag:Text id="loan_term_lower" name="loan_term_lower" displayLable="期限下限（月）" value='${data.loan_term_lower}' maxlength="14" isInt="true"/>
                    <webTag:Text id="loan_term_upper" name="loan_term_upper" displayLable="期限上限（月）" value='${data.loan_term_upper}' maxlength="14" isInt="true"/>
                </div>
                <div class="row">
                    <webTag:Text id="loan_amount_lower" name="loan_amount_lower" displayLable="额度下限（万元）" value='${data.loan_amount_lower}' maxlength="10" isDecimal="true"/>
                    <webTag:Text id="loan_amount_upper" name="loan_amount_upper" displayLable="额度上限（万元）" value='${data.loan_amount_upper}' maxlength="10" isDecimal="true"/>
                    <webTag:DataDictionarySelectTag sys_id="Phjf" id="source" name="source" displayLable="来源" parentCode="loan_source" value='${data.source}' defaultValue="bank" readonly="true"/>
                </div>
                <div class="row">
                    <webTag:Text id="loan_rate_lower" name="loan_rate_lower" displayLable="利率下限（年）" value='${data.loan_rate_lower}' maxlength="14" isDecimal="true"/>
                    <webTag:Text id="loan_rate_upper" name="loan_rate_upper" displayLable="利率上限（年）" value='${data.loan_rate_upper}' maxlength="14" isDecimal="true"/>
                ...
                <div class="row">
                    <webTag:DataDictionarySelectTag sys_id="Phjf" id="status" name="status" displayLable="状态" parentCode="loan_status" value='${data.status}' readonly="true" isRequired="true"/>
                    <webTag:Text id="order_num" name="order_num" displayLable="排序号" value='${data.order_num}' isInt="true"/>
                </div>
                <div class="row">
                    <webTag:TextareaTag id="loan_desc" name="loan_desc" rows="16" displayLable="产品说明:" value="${data.loan_desc}"/>
                </div>
                <div class="row">
                    <webTag:TextareaTag id="apply_condition" name="apply_condition" rows="16" displayLable="申请条件:" value="${data.apply_condition}" readonly="true"/>
                </div>
                <div class="row">
                    <webTag:TextareaTag id="apply_need_materials" name="apply_need_materials" rows="16" displayLable="所需材料:" value="${data.apply_need_materials}"/>
                </div>
                <div class="row">
