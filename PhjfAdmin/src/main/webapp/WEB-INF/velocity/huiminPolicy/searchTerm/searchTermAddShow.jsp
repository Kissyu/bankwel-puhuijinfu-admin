...
<div class="container-fluid" style="height: 650px; overflow-y: auto;">
    ...
    <!-- 添加信息数据区 start -->
    <div class="row-fluid">
        <form id="mainForm" class="form-horizontal alert alert-info fade in span12" action="/phjfht/api/v1/policy/goAddSimpSearchTermPage?type=${data.type}&parent_st_id=${data.parent_st_id}&parent_st_name=${data.parent_st_name}" method="POST">
            <!-- 提示信息标签 -->
            ...
            <fieldset>
                ...
                <div class="row">
                    ...
                    <webTag:ImgUploadTag id="logo" name="logo" displayLable="LOGO" value="${data.logo}" readonly="true"/>
                </div>
                <div class="row">
                    <webTag:Text id="treepath" name="treepath" displayLable="路径" value='${data.treepath}' isRequired="true" readonly="true"/>
                    <webTag:DataDictionarySelectTag sys_id="Phjf" id="is_child" name="is_child" displayLable="是否为子节点" parentCode="sys_policySearchIsChild" value='${data.is_child}' isRequired="true" readonly="true" />
                    <webTag:Text id="order_no" name="order_no" displayLable="序号" value='${data.order_no}' isRequired="true" isInt="true" maxlength="10" readonly="true"/>
                </div>
            </fieldset>
