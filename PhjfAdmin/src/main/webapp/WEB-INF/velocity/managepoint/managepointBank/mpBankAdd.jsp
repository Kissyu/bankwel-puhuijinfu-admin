...
<head>
    ...
    <script>
        ...
        $(function(){
            ...
            //办理点信息自动带出
            jQuery.validator.addMethod("getManagepoint",function(value,element) {
                var mpFlag = true;
                if (mp_code == value) {
                    return mpFlag;
                }
                var managepoint = CommonPubAjax.getManagepoint(value);
                $("#mp_name").val(CommonPubAjax.isEmpty(managepoint) ? "" : managepoint.name);
                $("#mp_contact").val(CommonPubAjax.isEmpty(managepoint) ? "" : managepoint.contact);
                mp_code = value;
                mpFlag = CommonPubAjax.isEmpty(managepoint) ? false : true;
                return mpFlag;
            },"办理点代码不存在，请重新录入。");
            ...
            jQuery.validator.addMethod("getBank",function(value,element) {
                ...
                $("#bank_contact").val(CommonPubAjax.isEmpty(bank) ? "" : bank.contact);
                bank_code = value;
...
<div class="container-fluid">
    ...
    <!-- 添加信息数据区 start -->
    <div class="row-fluid">
        <form id="mainForm" class="form-horizontal alert alert-info fade in span12" action="/phjfht/api/v1/managepoint/addMpBank" method="POST">
            <!-- 提示信息标签 -->
            ...
            <fieldset>
                <div class="row">
                    <webTag:Text id="mp_code" name="mp_code" displayLable="办理点代码" value='${data.mp_code}' isRequired="true" maxlength="32"/>
                ...
                <div class="row">
                    <webTag:Text id="bank_code" name="bank_code" displayLable="银行代码" value='${data.bank_code}' isRequired="true" maxlength="32" />
