...
var PageCtrl = $PC = function(){
	...
	var _tools = [{
		...
		iconCls:'icon-add',
		handler:function(){
			top.changeTab("银行业务配置管理", "银行业务配置管理","", path + '/phjfht/api/v1/bankinfo/goAddSimpBankBusinessConfigPage',false);
		}
	},'-',{
		id:'updateRow',
		text:'修改',
		...
		handler:function(){
			...
			top.changeTab("银行业务配置管理", "银行业务配置管理","", path + '/phjfht/api/v1/bankinfo/goModifyBankBusinessConfigPage?seq_uuid='+row.seq_uuid,false);
		}
	},'-',{
		id:'viewRow',
		text:'查看',
		...
		handler:function(){
			...
			top.changeTab("银行业务配置管理", "银行业务配置管理","", path + '/phjfht/api/v1/bankinfo/goViewBankBusinessConfigPage?seq_uuid='+row.seq_uuid,false);
		}
	}];
	...
	return{
		...
		renderUI:function(){
			$CL.datagrid({
    			url:'/phjfht/api/v1/bankinfo/queryBankBusinessConfigByPage',
   				queryParams: {
					language:"ZH_SIMP"
        		},
    			fit: true,
			    ...
			    columns:[[
					...
					,{field:'device_type_name', title:'设备类型',width:40}
					,{field:'app_version', title:'版本号',width:40}
